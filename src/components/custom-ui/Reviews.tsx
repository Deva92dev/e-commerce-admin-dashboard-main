    ...
    async (pageNum: number) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const updatedReviews = newReviews.filter(
          ...
          reviewsRef.current = [...reviewsRef.current, ...updatedReviews];
          setReviews(reviewsRef.current);

          setHasMore(
            newReviews.length > 0 && reviewsRef.current.length < totalReviews
          );

          // Check if the user has already submitted a review
          const userHasReviewed = newReviews.find(
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {reviewsRef.current.length > 0 ? (
          reviewsRef.current.map((review: ReviewType) =>
